...
//				svgEditor.canvas.addSvgElementFromJson(element);
//		});

function getSVG()	{
	return svgEditor.canvas.svgToString(svgcontent, 0);
}

function setSVG(xml)	{
	svgEditor.canvas.setSvgString(xml);
}

svgEditor.addExtension("Web Socket", function() {
				...
				switch(shapeToSend.element) {
					...
					case 'text' :
						shapeToSend.attr = {
							...
							style: element2.getAttribute("style"),
							opacity: element2.getAttribute("opacity"),
							fontSize: element2.getAttribute("font-size"),
							fontFamily: element2.getAttribute("font-family"),
							transform: element2.getAttribute("transform"),
							textAnchor: element2.getAttribute("text-anchor")
						};
						shapeToSend.curStyles = true;
...
//				socket.emit('update', {svg:shapeToSend});
		...
		elementRemoved: function(opts)	{
			...
			removedElements.forEach(function(elementToRemove, index, array) {
